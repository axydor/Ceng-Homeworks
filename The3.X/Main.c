...
int result2 = 0; // The result of the A/D conversion to check whether User change POT, if not turned the POT blink '#'
int blink_index = 0;  // Variable to know which number to blink 
int pins_setted = 0; // 
int show_flag = 1; // Flag for showing the setted pin with 500 ms delays
int show_c = 0;   
int show_three = 0;  // Counter up to 6, 500ms * 3 = 6 seconds. Every half second it is incremented by one
int second_c = 0; // It will count up to 200 to make one sound
int seconds = 120;
int seg_ind = 0;  // Index of 7-seg-display

char pass[5] = "####"; // 
char attempt[5] = "####";
char blink[5] = "1111"; // Whether we will blink the related blink_index or not

int waited_3_sec = 0; // It will be set for waiting 3 seconds, after pushing RE1
int re1_pressed = 0, set_flag = 0;
int a = 0, temp = 0; // dummy, but very dummy variable
char no; // Each number of the password
int b = 0; // dummy
int x=0; // Variable for executing inside of enter_pin() only once
int temp_portb;
...
void init_interrupts();      //  Initialize the interrupts after pushing RE1 and waiting 3 seconds 
void up_write(int i, char c); // Writes the given* char to the i'th position in the Upper Side of LCD. (*): ' Set a pin:####'
void show_passwd();           // Shows the password for 3 seconds with 500ms delays
void write_message(char* str, int up);  // Helper function for writing the given string to upper or down side of LCD
...
*  After the set a pin screen comes, we wait for A/D change and RB6 pushes.
*  In this stage, We used blink_the_index which calls the corresponding wink(i) function to blink '#'
*  User can press RB6 many times, it will come to first index again
*  After RB7 push, program shows the entered pin for 3 seconds with 500 milliseconds delay which
*  was generated by TIMER0. In the pin entering stage, after attempting two consecutive fails, program
*  freezes for 20 seconds and then user can attempt again   
*/

/* PROBLEMS : 7-segment display is slightly flickering while counting down, Interestingly It is not flickering
*  after entering 2 wrong pins and waiting 20 seconds locks 
*
*/
...
void seven_seg_display() {
    ...
    if(show_three < 6 && waited_3_sec == 3)
    {
    	PORTH = 1;
    	PORTJ = DASH;
    	delay_ms(1);
...
void show_passwd() {
    if (show_three == 6) {
        ...
        TMR1L = 200;
        TMR1H = 60;
        T1CON = 0b00000001; // 16-bit mode, Enable
        PIE1bits.TMR1IE = 1;
        INTCONbits.PEIE = 1;
...
// Blink the index of the pin accordingly
void blink_the_index(int blink_index, char* str) {
    switch (blink_index) {
        case 0:
            wink(str, 0);
...
void updateLCD() {
    if (waited_3_sec == 1) {
        ...
        delay_3();
        ClearLCDScreen();
        return; // WE return here so that do not pass below
    }
    if (set_flag == 1 && (pins_setted < 1) && pins_setted != -1) { // We are setting the password
        INTCONbits.GIE = 0;
        blink_the_index(blink_index, pass);
        INTCONbits.GIE = 1;
    }
...
void init_interrupts() {
    ...
    INTCONbits.TMR0IE = 1; // ENABLE TIMER0 INTERRUPTS
    T0CON = 0b11010111;    // TIMER0 ON, 8 bit-mode, 1:256 prescale
